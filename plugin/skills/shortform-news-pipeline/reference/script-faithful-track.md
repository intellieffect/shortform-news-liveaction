...
3. **슬롯이 먼저, 그다음 실사 > 생성.** 라이선스가 명시된 외부 소스만(기관 CC BY/PD → 스톡 → 생성). **그런데 이 순서는 슬롯마다 다르다** — `layer43_plan[b].slot` 을 먼저 정한다.
   ...
   실물 없으면 "같은 원리의 실물 + 실물 아님 라벨". **8편 실측**: 톤 배경을 스톡으로 버티다 b13 크롭 2회·파문 3회를 고쳤고 그게 P7 재판정 2라운드의 상당 부분이 됐다 — 생성 1클립은 5~12 크레딧·2분이다. ([근거](../../../../docs/research/2026-09-03-generation-overbrake/summary.md))
