...
3. **전체 이야기의 작업 가설:** 먼저 [공통 표현 레퍼런스](visual-references.md)의 실제 세트를 확인하여 표현 선택과 완성도 기준을 이해한다.  이번 기사와 실제 자료에서 시청자가 관심을 가질 이유와 끝까지 따라갈 발견을 정한다. `story.question`과 `story.takeaway`는 그 방향을 압축한 작업 값이다. 처음 생긴 궁금증이 설명을 거치며 어떻게 이어지거나 바뀌고, 끝에서 무엇을 새롭게 이해하게 되는지 원고와 개념 순서로 설계한다. 준비 설명의 양은 그 흐름에 필요한 만큼 정한다. 기승전결의 기능을 설계하되 고정된 도입 시간·반전·컷 수를 요구하지 않는다.
...
5. **자료와 표현:** [시각 표현 안내](visual-expression.md)를 읽고 이미지·일러스트·인용·표·그래프·도해·모션그래픽을 자유롭게 조합한다. 필요한 외형 자산은 Higgsfield로 제작해 코드와 합성하며 사용 빈도·배치를 고정하지 않는다. [자료 선택과 수집](../../shortform-news-input/reference/sourcing.md)에 따라 전체 목적을 소싱 담당에게 전달한다. 확보한 실물의 내용·움직임·길이가 원고와 장면을 어떻게 살리는지 보고 표현을 선택한다. 자료가 부족하거나 예상한 효과가 약하면 검색 범위·표현 수단과 함께 이야기 구성도 재검토한다. 개념마다 evidence/metaphor/tone 역할과 footage/image/diagram/hybrid/text 수단, 선택 이유를 기록한다.
각 핵심 설명의 대상·작용·결과는 **화면에서 관찰될 것**으로 적고, 필요한 외형과 필요한 움직임을 먼저 나눠 생성·코드 역할을 구현 전에 정한다([발화→화면 제작](visual-production.md)). 레퍼런스에서 이번 편에 적용할 판단은 기존 제작 노트에 짧게 남긴다.

사실 검토나 수정으로 대상·값·비교 관계가 달라지면, 선택한 표현이 수정된 내용을 여전히 명확하게 전달하는지 다시 확인하고 적용 판단도 갱신한다. 예를 들어 비교 대상 하나를 제외했다면 남은 축·막대·배치가 이제 무엇을 설명하는지 본다. 남은 범위를 읽히는 데 적합하면 유지하고, 설명할 관계를 잃었다면 표현을 다시 선택한다. 변경 이유는 기존 제작 노트의 해당 판단에 반영한다.

새 편의 첫 핵심 장면 시험(`first-core-scene@5`)은 **선택이며 음성 착수 게이트가 아니다.** 원고·주요 자료·표현 방향이 정해지면 6단계 음성과 11단계 전체 시안으로 진행한다. 그중 하나의 선택을 실제로 바꿀 구체적 불확실성이 있을 때만, 그 질문에 답할 만큼의 합성 이미지나 동작 시안을 만든다. 단순 미술 조정은 시험 사유가 아니다. 외형과 합성 가능성은 제작자가 직접 확인하고, 독립적인 의미·오독 관찰이 필요할 때만 `scene-judge`를 부른다([review-loop.md](review-loop.md#초기-첫-장면)). 설계한 형태·궤적은 가설이며 사실과 의미가 보존되면 제작자가 바꿀 수 있다. 시험 질문, 진행 판단, 실패 시 대안은 기존 제작 노트에 짧게 남기고 별도 기획 승인 서식을 만들지 않는다. 이 시험은 전체 영상의 승인이 아니며 이야기·호흡·음향 판정은 11단계 독립 검수에 남긴다. 계약 전문은 [발화→화면 제작](visual-production.md)과 [docs/SCENE-PROOF.md](../../../../docs/SCENE-PROOF.md)에 있다. `@1`~`@4`로 시작한 편은 기록된 게이트를 그대로 유지한다.
...
11. **전체 시안·독립 검수·수정:** `resume`의 `context.work`에서 현재 시안과 관찰을 확인하고 [review-loop.md](review-loop.md)의 입력 명령으로 P5/P7 검수를 연결한다. 실물에서 읽힌 흐름을 먼저 받고 의도와 대조한다. 첫 관찰은 원문 그대로 보존하고 요약해 옮기지 않는다. 제작자는 중요한 관찰에 대해 자료·순서·원고·모션·음향 중 필요한 수정 범위를 선택해 실행하거나 유지 이유를 남긴다. 변경된 결과를 갱신하고 원래 문제가 어떻게 달라졌는지 재확인한다. 초기 시안이 충분하면 불필요한 재수정을 하지 않는다.
...
기준편은 [golden-editorial.json](golden-editorial.json)의 베텔게우스·은하 뒤집힘·우주택배·로먼이다. 이 네 편은 품질과 데이터 역검증의 기준이며 신규 제작 시간 단축의 근거가 아니다. 기준편 영상과 개선 기록(`docs/VIDEO_IMPROVEMENTS.md`)은 제작사 로컬 평가 자료라 저장소에 싣지 않는다 — 없으면 이 절은 건너뛰고, 제작 입력은 공통 표현 레퍼런스(`references/visual/`)를 쓴다.

독립 제작 검증은 새 세션에서 현재 기사·사용자 조건·실제 자료·기술 문서로 진행한다. 지정된 공통 표현 레퍼런스는 정상 제작 입력으로 제공한다. 임의의 과거 원고·정답·개별 수정 지시를 이번 편의 정답으로 복제하지 않는다. 평가용 기준편은 제작 뒤 평가 측에서 비교한다. 레퍼런스를 사용한 제작을 레퍼런스 없는 독립 제작으로 보고하지 않는다. 전달한 자료, 버전·모델 설정·자료 접근 조건·사용자 개입을 함께 기록한다. 동일 기사 비교와 다른 새 기사 검증의 범위를 구분한다.
