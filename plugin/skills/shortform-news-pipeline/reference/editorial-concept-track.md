...
핵심 설명은 같은 계약에 따라 음성 확정 전에 합성 이미지, 필요한 경우 동작 시안으로 확인한다. 이 초기 확인은 문서 계획·코드 존재로 대신하지 않는다. 확인 대상은 **첫 핵심 장면 한 개**이며 전체 영상의 승인이 아니다 — 여기에 새 사용자 승인 단계를 만들지 않고, 이야기·호흡·음향 판정은 11단계 독립 검수에 남긴다.
