...
const read = (w, name) => existsSync(join(w.production, name)) ? json(join(w.production, name)) : {};
...
export function firstSceneReadiness(w, state) {
  ...
  if (config.schema !== 'scene-proof-config@1' || !selected) add('first-scene-selection', 'scene-proof.json에서 기사 핵심 개념 하나와 공통 합성 경로를 선택한다');
  ...
  try {
    if (typeof config.component !== 'string' || !config.component.startsWith('src/editorial/scenes/') || !config.component.endsWith('.tsx') || !existsSync(w.path(config.component))) add('first-scene-component', '실제 재사용 장면 컴포넌트가 필요하다');
  ...
  const plan = validateVisualPlan({concepts: {concepts: [selected]}, visualSystem: visual, required: true});
  ...
  for (const phase of ['still', ...((selected.visual?.moments ?? []).some(m => m.motion_required) ? ['motion'] : [])]) {
    ...
    if (render?.kind !== 'shared-scene-proof@1' || render.config !== `news/${w.id}/02_production/scene-proof.json` || render.component !== config.component || JSON.stringify([...(render.asset_ids ?? [])].sort()) !== JSON.stringify([...ids].sort()) || render.profile_sha256 !== hash(readFileSync(join(w.repo, 'config/production-profile.json')))) add('first-scene-composite', `${phase}: 현재 실제 자산·장면 컴포넌트·공통 자막을 쓴 시안이 필요하다. scripts/scene-proof.mjs 사용`);
