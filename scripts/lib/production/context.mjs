...
export const productionContext = (w, state, actions, completion) => {
  ...
  return {
    ...
    tools: {
      ...
      narration: `npm run narration:assemble -- ${w.id} --alignment 02_production/audio/alignment.json`,
      assets: `npm run fetch:assets -- news/${w.id} --manifest 02_production/sourcing/video/assets.json --jobs 4`,
      note: "spec/preflight는 필요한 규격·실패만 조회한다. 명령의 선택 인자와 담당 manifest는 실제 작업에 맞게 지정한다. 새 begin/finish·독립 검수 권한을 대신하지 않는다.",
    },
    ...
    work: {
      ...
      refresh: Object.entries(actions).filter(([, action]) => action.status !== "current" && !action.optional).map(([action, value]) => ({ action, status: value.status, reasons: value.reasons, runnable: value.runnable })),
