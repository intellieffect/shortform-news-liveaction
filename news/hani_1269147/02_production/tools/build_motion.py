...
def ev(id,concept,element,kind,f,s,t,e,why,easing=("ease-out","ease-in-out","ease-in"),dep=[],co=[]):
    E.append({"id":id,"concept_id":concept,"element_id":element,"kind":kind,"timing":{"from":f,"settled":s,"to":t,"end":e},"easing":{"enter":easing[0],"move":easing[1],"exit":easing[2]},"depends_on":[d if isinstance(d,dict) else {"event_id":d} for d in dep],"coexist_with":co,"why":why})
L=last
# leak s01-s03
...
ev("light_label","model","light_label","label",A("s08","가벼운",'start',-2),A("s08","가벼운",'start',10),A("s09",L("s09"),'end',-2),A("s09",L("s09"),'end',6),"가벼운 기체를 말할 때 위층 라벨")
ev("heavy_label","model","heavy_label","label",A("s08","무거운",'start',-2),A("s08","무거운",'start',10),A("s09",L("s09"),'end',-2),A("s09",L("s09"),'end',6),"무거운 기체를 말할 때 아래층 라벨")
# observation s10-s12
...
ev("telescope_label","observation","telescope_label","label",A("s10","이천이십사년,",'start',2),A("s10","이천이십사년,",'start',12),A("s10","행성이",'start',-6),A("s10","행성이",'start',2),"망원경 이름·장소·연도",dep=[{"event_id":"telescope_show","point":"from"}])
...
ev("spectrum_label","observation","spectrum_label","label",A("s11","펼치면,",'start',0),A("s11","펼치면,",'end',6),A("s12",L("s12"),'end',-2),A("s12",L("s12"),'end',8),"띠가 무엇인지 표기",dep=[{"event_id":"spectrum_show","point":"from"}])
ev("helium_dip","observation","helium_line","diagram",A("s12","그",'start',-2),A("s12","나타났습니다.",'end',-4),A("s12",L("s12"),'end',-2),A("s12",L("s12"),'end',8),"띠의 한 지점이 어두워져 선이 남는다",dep=[{"event_id":"spectrum_show","point":"from"}])
...
ev("model_note2","meaning","model_note2","label",A("s14","새는",'start',-2),A("s14","새는",'start',8),A("s14","기체뿐.",'end',20),A("s14","기체뿐.",'end',28),"'가장 가벼운 기체만 샌다'는 모델 예측 표기")
ev("heavy_layer","meaning","heavy_layer","diagram",A("s14","남아",'start',-2),A("s14","있고,",'end',0),A("s15",L("s15"),'end',-2),A("s15",L("s15"),'end',8),"아래 남은 무거운 기체층 띠")
ev("ocean_condition","meaning","ocean_condition_text","label",A("s14","온실효과를",'start',-2),A("s14","온실효과를",'start',10),A("s15","이",'start',-6),A("s15","이",'start',2),"조건 문구",dep=["heavy_layer"])
ev("ocean_value","meaning","ocean_value","label",A("s14","액체",'start',-2),A("s14","액체",'start',10),A("s15","이",'start',-6),A("s15","이",'start',2),"바다 가능성 값 — 조건과 완전 표시 구간이 겹친다",dep=["ocean_condition"])
